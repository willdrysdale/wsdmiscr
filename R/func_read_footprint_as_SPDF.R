...
read_footprint_as_SpatialPointsDataFrame = function(filepath,dimpath,gz_file = T){
  #If file is a gzfile, decompress and read
  if(gz_file)
    d = read.csv(gzfile(filepath),header = F)
  #else just read.csv
  else
    d = read.csv(filepath,header = F)
  #load dimentions  
