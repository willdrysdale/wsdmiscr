...
create_nasa_ames_1D = function(d,
                               ONAME,
                               ORG,
                               SNAME,
                               MNAME,
  ...
  for(i in 1:length(VNAMEn)){
    if(nchar(VNAMEn[i]) > 132)
      stop("A VNAMEn entry is longer than 132 characters")
  }
  #format date 
  DATE = str_replace_all(as.character(ymd(DATE)),"-"," ")
  RDATE = str_replace_all(as.character(Sys.Date()),"-"," ")
